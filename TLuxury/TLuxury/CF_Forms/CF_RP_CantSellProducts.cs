...
{
    public partial class CF_RP_CantSellProducts : Form
    {
        public CF_RP_CantSellProducts()
        {
            InitializeComponent();
            WireData();
        }
        private void WireData()
        {
            try
            {
                ...
                {
                    DataTable table = GlobalConfig.Connection.ReportCantSellProduct(int.Parse(comboBoxThang.Text));
            ...
            {
                MessageBox.Show("Lỗi Gì đó k biết đươc :(");
            }
        ...
        private void buttonbaoCao_Click(object sender, EventArgs e)
        {
            if (DanhsachSP.Rows.Count > 1)
            {
                ...
                string TieudeBaoCao = $"Báo Cáo Danh Sách Sản Phẩm Không Bán Được Trong Tháng {comboBoxThang.Text}";
                //tạo đối tượng excel
